...
public class PhotoMetadataReaderTests
{
    ...
    [Fact]
    public void Reads_the_camera_model_written_by_a_real_encoder()
    {
        var path = Path.Combine(Path.GetTempPath(), $"{Guid.NewGuid()}.jpg");
        TestImages.WriteJpeg(path, width: 100, height: 100, camera: "NIKON D50");

        try
        {
            var metadata = PhotoMetadataReader.Read(path);

            Assert.Equal("NIKON D50", metadata.Camera);
        }
        finally
        {
            File.Delete(path);
    ...
    [Fact]
    public void Reports_no_camera_when_the_file_carries_none()
    {
        var path = Path.Combine(Path.GetTempPath(), $"{Guid.NewGuid()}.jpg");
        TestImages.WriteJpeg(path, width: 100, height: 100);
