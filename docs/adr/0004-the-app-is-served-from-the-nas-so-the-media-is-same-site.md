...
**Reloading collapses that cascade into one prompt, and this was measured too.** With the `.htpasswd` line restored, a page reload produced a single document-level dialog; entering the credential once restored every subresource, and a full probe run afterwards came back clean. So the recovery path this ADR promised does exist — it is a reload, not a dismissal. What the build has to add is the trigger: an authenticated probe before rendering, and `location.reload()` on a `401`, so the app never sits in the open-page state where the dialogs arrive one at a time. That is a handful of lines in the shell, outside the authorization path, and it does not disturb the "no application code" claim about the threshold itself.
